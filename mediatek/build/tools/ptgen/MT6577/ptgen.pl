...
my %productMkPartition; # Partition size read from PRODUCT MAKEFILE
...
$MTK_HYBRID_PRODUCTFILE=$ENV{MTK_HYBRID_PRODUCTFILE};
...
if (($MTK_HYBRID_PRODUCTFILE ne "") && (-e $MTK_HYBRID_PRODUCTFILE)){
	&ReadProductMakefile () ;
}
...
#****************************************************************************
# subroutine:  ReadProductMakefile
# return:      
#****************************************************************************
sub ReadProductMakefile () {
	open (PRODUCTMK, "< $MTK_HYBRID_PRODUCTFILE") or &error_handler("Ptgen open PRODUCT MAKEFILE fail!\n", __FILE__, __LINE__);
	while (<PRODUCTMK>) {
		next if (/^\s*#/);
		$productMkPartition{"RECOVERY"} = $1 if (/BOARD_RECOVERYIMAGE_PARTITION_SIZE\s*:=\s*(\S+)/);
		$productMkPartition{"USRDATA"} = $1 if (/BOARD_USERDATAIMAGE_PARTITION_SIZE\s*:=\s*(\S+)/);
		$productMkPartition{"ANDROID"} = $1 if (/BOARD_SYSTEMIMAGE_PARTITION_SIZE\s*:=\s*(\S+)/);
	}
	close PRODUCTMK;
}
...
sub ReadExcelFile()
{
	...
	while($pt_name ne "END"){
		...
		if (exists $productMkPartition{"$pt_name"}) {
			my $temp_size;
			my $real_value = $productMkPartition{"$pt_name"} / 1024;
			my $int_value = int ($productMkPartition{"$pt_name"} / 1024);
			$temp_size = $int_value + 1 if ($real_value != $int_value);
			$temp_size = $real_value if ($real_value == $int_value);

			$temp_size += 1*1024 if (($pt_name eq "SEC_RO") || ($pt_name eq "ANDROID") || ($pt_name eq "CACHE"));
			$temp_size += 2*1024 if ($pt_name eq "USRDATA");
			$SIZE_FIELD_KB[$row_t-1] = $temp_size;
		} else {
