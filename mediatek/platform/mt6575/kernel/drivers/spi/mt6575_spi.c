...
static int clear_resume_bit(struct mt6575_spi *mtspi)
{
	u32 reg_val;
	unsigned long flags;
...
static int spi_end_transfer(struct mt6575_spi *mtspi)
{
	u32 reg_val;
	unsigned long flags;
...
{	
	...
	if ( trans->tx_buf ) {
		trans->tx_dma = dma_map_single(dev, (void *)trans->tx_buf, 	
	...
	if (trans->rx_buf) {
		trans->rx_dma = dma_map_single ( dev, 	trans->rx_buf, 
		...
		if ( dma_mapping_error ( dev, 
			trans->rx_dma ) ) {
			if ( trans->tx_buf ) 
				dma_unmap_single ( dev, trans->tx_dma, 
					trans->len, DMA_TO_DEVICE );
	...
	struct spi_transfer *trans )
{
	...
	if ( trans->rx_dma != INVALID_DMA_ADDRESS )
		dma_unmap_single ( dev,  trans->rx_dma, 
			trans->len, DMA_FROM_DEVICE );
...
{	
	...
	if ( ( ( reg_val & 0x30000 ) >> 16 ) )
		wait_for_completion_timeout ( &done, 5*HZ);
	...
	struct spi_transfer *transfer )
{
	...
	do {
		...
		dma_reg = spi_readl ( mtspi, SPI_CFG1_REG );
		dma_reg &= ~SPI_CFG1_PACKET_LENGTH_MASK;
		dma_reg |= ( packet_size - 1 ) << SPI_CFG1_PACKET_LENGTH_OFFSET;
		SPI_DBG("SPI_CFG1_REG is:0x%x\n", dma_reg );
		spi_writel ( mtspi, SPI_CFG1_REG, dma_reg );
		...
		SPI_DBG("The packet_loop is: 0x%x\n", packet_loop);

		/*set the number of the packets in this message.*/
		dma_reg = spi_readl ( mtspi, SPI_CFG1_REG );
		dma_reg &= ~SPI_CFG1_PACKET_LOOP_MASK;
		dma_reg |= ( packet_loop-1 ) << SPI_CFG1_PACKET_LOOP_OFFSET;
		SPI_DBG("SPI_CFG1_REG is:0x%x\n", dma_reg);
		spi_writel(mtspi, SPI_CFG1_REG, dma_reg);

		/*set the buffer and then start transfer*/
		if ( mtspi->cur_transfer->tx_buf  ) {
			spi_writel ( mtspi, SPI_TX_SRC_REG, 
				cpu_to_le32 ( mtspi->cur_transfer->tx_dma ) );
			SPI_DBG ( "cpu_to_le32: 0x%x\n", 
				cpu_to_le32 ( mtspi->cur_transfer->tx_dma ) );
			dma_reg = spi_readl(mtspi, SPI_TX_SRC_REG);
			SPI_DBG ( "The SPI_TX_SRC_REG is:0x%x", dma_reg );
		}
	
		if ( mtspi->cur_transfer->rx_buf ) {
			spi_writel ( mtspi, SPI_RX_DST_REG, 
				cpu_to_le32 ( mtspi->cur_transfer->rx_dma ) );
			dma_reg = spi_readl ( mtspi, SPI_RX_DST_REG );
			SPI_DBG ( "The SPI_RX_DST_REG is:0x%x\n", dma_reg );
		}
		...
		if ( mtspi->cur_transfer->tx_buf ) {
			dma_reg = spi_readl ( mtspi, SPI_CMD_REG );
			dma_reg |= 1 << SPI_CMD_TX_DMA_OFFSET;
			spi_writel(mtspi, SPI_CMD_REG, dma_reg);
		}
		if ( mtspi->cur_transfer->rx_buf ) {
			dma_reg = spi_readl ( mtspi, SPI_CMD_REG );
			dma_reg |= 1 << SPI_CMD_RX_DMA_OFFSET;
			spi_writel(mtspi, SPI_CMD_REG, dma_reg);
		}
	...
	struct spi_transfer *transfer )
{
	...
	do {
		...
		/*set the number of packet for this message.*/
		reg_val = spi_readl ( mtspi, SPI_CFG1_REG );
		...
		for ( i = 0; i < cnt; i++ ) {
			spi_writel ( mtspi, SPI_TX_DATA_REG, 
				*( ( u32 * ) mtspi->cur_transfer->tx_buf + i ) );
	...
	struct spi_transfer *transfer )
{
	...
	if ( mtspi->config->com_mod == OTHER1 ) {
		...
		transfer->tx_dma = dma_map_single(
			&mtspi->pdev->dev,   
			(void *)transfer->tx_buf,  
			transfer->len, 
			DMA_TO_DEVICE);
		...
		if ( dma_mapping_error (&mtspi->pdev->dev,  
			transfer->tx_dma ) ) {
			SPI_DBG("dma mapping tx_buf error.\n");
			return -ENOMEM;	
		}	
		...
		dma_unmap_single ( &mtspi->pdev->dev, 
			transfer->tx_dma, 
			transfer->len, 
			DMA_TO_DEVICE );
		...
		for ( i = 0; i < cnt; i++ ) {
			spi_writel ( mtspi, SPI_TX_DATA_REG, 
				*( ( u32 * ) transfer->tx_buf + i ) );
		}

		transfer->rx_dma = dma_map_single (
			&mtspi->pdev->dev, 
			transfer->rx_buf, 
			transfer->len, 
			DMA_FROM_DEVICE );
	...
	struct spi_message *msg )
{
	...
	mtspi->cur_transfer = list_entry ( msg->transfers.next, 
		struct spi_transfer,  transfer_list );
	...
	else if ( mtspi->config->com_mod == OTHER1||
		mtspi->config->com_mod == OTHER2 )
...
static int mt6575_spi_setup ( struct spi_device *spidev )
{
	...
	if (spidev->chip_select >= master->num_chipselect) {
		SPI_DBG("spi device chip select excesses \
	...
	if ( !chip_config ) {
		...
		dev_dbg ( &spidev->dev, 
			"device %s: allocated memoy for controller's runtime state\n", 
			dev_name ( &spidev->dev ) );
	...
	/*
	if (chip_config->cpha != pha)
		SPI_DBG("Parameter cpha is different from pha of this device.\n");
	if (chip_config->cpol != pol)
		SPI_DBG("Parameter cpol is different from pol of this device.\n"); */
...
static int __init mt6575_spi_probe ( struct platform_device *pdev )
{
	...
	ret = request_irq ( irq, 
		mt6575_spi_interrupt, IRQF_TRIGGER_LOW, 
