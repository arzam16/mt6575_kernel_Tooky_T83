	...
	union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CAPACITY:
	    ...
	    //val->intval = 100 - ((4200 - data->BAT_batt_vol) / 8);	// Add Zormax    
	    break;        
...
static void mt6575_battery_update(struct mt6575_battery_data *bat_data)
{
    ...
    {     
        ...
        {
            /* Battery Full */ 
            if ( (BMT_status.bat_vol >= RECHARGING_VOLTAGE) && (BMT_status.bat_full == KAL_TRUE) && (g_HW_Charging_Done == 1) )
            {
            	/*Use no gas gauge*/
			...
			{					
				...
				{		
					...
					if(gFGsyncTimer >= DEFAULT_SYNC_TIME_OUT)
					{
						...
						//bat_volt_check_point = BMT_status.SOC;
						//bat_data->BAT_CAPACITY = bat_volt_check_point;
						//bat_data->BAT_CAPACITY =  100 - ((4200 - bat_data->BAT_batt_vol) / 8);
					}
					else
					{
						gFGsyncTimer+=10;
...
UINT32 BattVoltToPercent(UINT16 dwVoltage)
{
    ...
    { 
        //bPercntResult =  100 - ((4200 - dwVoltage) / 8); // Add Zormax
        //return bPercntResult; // Add Zormax
...
int BAT_CheckBatteryStatus(void)
{
	...
	else
	{
		/* Re-calculate Battery Percentage (SOC) */    
		//BMT_status.SOC = FGADC_Get_BatteryCapacity_CoulombMothod();
		BMT_status.SOC = FGADC_Get_BatteryCapacity_VoltageMothod(); // Add Zormax
		//BMT_status.SOC = BattVoltToPercent(BMT_status.bat_vol); // Add Zormax
...
PMU_STATUS BAT_ConstantCurrentModeAction(void)
{
	...
	if ( BMT_status.bat_vol > V_CC2TOPOFF_THRES )
	{
		...
		{
			for (i=0; i<BATTERY_AVERAGE_SIZE; i++) {	            
				batteryCurrentBuffer[i] = AC_CHARGER_CURRENT;
        	}
		...
		{
			for (i=0; i<BATTERY_AVERAGE_SIZE; i++) {	            
				batteryCurrentBuffer[i] = USB_CHARGER_CURRENT;
        	}
