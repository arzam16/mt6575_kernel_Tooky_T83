...
#define BATTERY_OVER_TEMP               0x02    /*bit1 set. jrd add for recovery charging from high temperature */
#define BATTERY_LOW_TEMP                0x04   /*bit2 set.*/
...
void fan5405_set_ac_current(void)
{
    ...
    fan5405_config_interface_liao(0x06,0x7a); //set ISAFE :1250mA, VSAFE:4.42V
    fan5405_config_interface_liao(0x01,0xC8);
    fan5405_config_interface_liao(0x02,0x8E);
    fan5405_config_interface_liao(0x04,0x29); //set IOCHARGE = 1250mA-->650mA P18, Iterm = 97mA
    fan5405_config_interface_liao(0x05,0x04);
    #endif
}
...
void select_charging_curret_fan5405()
{
    ...
    {        
        ...
        fan5405_config_interface_liao(0x04,0x09); //set IOCHARGE= 550mA, Iterm = 146mA --> 97mA
		if (Enable_BATDRV_LOG == 1) {
...
void ChargerHwInit_fan5405(void)
{
	...
	if(temp_init_flag==0)
	{
	    fan5405_config_interface_liao(0x06,0x7a);	/*ISAFE:1250mA, VSAFE:4.2V*/
	    fan5405_config_interface_liao(0x00,0xC0);
	    fan5405_config_interface_liao(0x01,0xb1);
	    fan5405_config_interface_liao(0x02,0x8e);
	    fan5405_config_interface_liao(0x05,0x04);
	    fan5405_config_interface_liao(0x04,0x29);	/*Iocharge:0x02 : actually 650mA ?*/
	    temp_init_flag =1;	
	}
	else
	{
...
#if 0 	/*jrd delete for display charging animation when bat_full flag not set*/
...
#if 1        
        /* UBOOT charging LED */
#if 0	/*jrd delete for leds controlled by GPIO67*/
...
/*jrd add end*/
    ...
    if (upmu_is_chr_det() == KAL_TRUE)
    {
        if ((BMT_status.bat_vol < RECHARGING_VOLTAGE) && (BMT_status.bat_full) && (g_HW_Charging_Done == 1))	//4110mV
        {
            //if (Enable_BATDRV_LOG == 1) {
...
void check_battery_id(void){
  ...
  if((bat_id_vol < CHG_MIN_VALID_BATT_ID) || (bat_id_vol > CHG_MAX_VALID_BATT_ID)) /*battery id error*/
	...
	pchr_turn_off_charging_fan5405();
        ...
        mt_set_gpio_mode(GPIO67, GPIO_MODE_00);         /*charging indicator led off on Griffin PWM1 pin(GPIO67)*/
        mt_set_gpio_dir(GPIO67, GPIO_DIR_OUT);
        mt_set_gpio_out(GPIO67, GPIO_OUT_ONE);
   ...
   // if(upmu_chr_get_vcdt_hv_det() == 1 || (upmu_is_chr_det() == KAL_FALSE))
    ...
    else
    {
    	...
    	if(( (fan5405_status == 0x2) && (fan_charger_status == CHARGER_IN_PROGRESS_STATUS) ) || 
				((BMT_status.TOPOFF_charging_time > 60*55) && (BMT_status.bat_charging_state == CHR_TOP_OFF)))
    	{
    		BMT_status.bat_charging_state = CHR_BATFULL;
    		BMT_status.bat_full = KAL_TRUE;
    		BMT_status.total_charging_time = 0;
	...
	if(BMT_status.bat_vol >= TOPOFF_CHG_VOLTAGE)	//4080mV
	{
	BMT_status.bat_charging_state = CHR_TOP_OFF;
	BMT_status.CC_charging_time = 0;
...
void bmt_charger_ov_check(void)
{
     ...
     fan_status = fan5405_get_chip_status();	/*read con0*/

     if((upmu_chr_get_vcdt_hv_det() == 1) || (fan_status == 0x03) || 
					(fan_charger_status == CHARGER_FAULT_NOW))	/*pmic vcdt high > 6.5V, or fan5405 fault*/
#else
...
#endif
     {
		...
		{
			check_ret = check_fan5405_fault_status(fan_fault_status);	/*check or recovery from fault status*/
			if(check_ret == 0)
				{
				charger_ov_boot_display = 0;
				break;	/*recovery from fault status*/
				}
		}	 
#endif
                  #ifdef GPT_TIMER                  
...
void uboot_charging_display()
{
    ...
    if ( (BMT_status.bat_full) || (user_view_flag == KAL_TRUE) || (g_HW_Charging_Done == 1)) 
    {		
        if(g_bl_on == KAL_TRUE)
        {	
            mt65xx_disp_show_battery_full();	
        }
        user_view_flag = KAL_TRUE;	/*only this way to set this flag, it means when battery full once time, it always display full gauge*/
    } 
    else 
    {	
	...
	if(fan5405_get_chip_status() == 0x00)
		...
		if(fan_charger_status != CHARGER_IN_PROGRESS_STATUS)
                ...
                {	
                    ...
                    #endif			
                    upmu_chr_baton_tdet_en(1);  // check me
		    ...
		    if(fan_charger_status != CHARGER_IN_PROGRESS_STATUS)	
        	    ...
        	    mt_set_gpio_mode(GPIO67, GPIO_MODE_00);	/*power-on with charger on --> led on*/
	            mt_set_gpio_dir(GPIO67, GPIO_DIR_OUT);
               	    mt_set_gpio_out(GPIO67, GPIO_OUT_ZERO);
