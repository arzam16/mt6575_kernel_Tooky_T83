...
static long apds9190_initialize ( struct i2c_client *client  )
{
    ...
    apds9190_set_pers ( client, 0x33 ); /* Important : if you use set to "0", you may see un-wanted interrupt caused by noise */
    apds9190_set_config ( client, 0 );
    apds9190_set_ppcount ( client, 0x07 ); /* 7-Pulse for proximity */
    apds9190_set_control ( client, 0x20 ); /* 100mA, IR-diode, 1X PGAIN, 1X AGAIN */

    /* crosstalk value shall be set by LGP Server using I/O so init here to zero */
    obj->ps_cross_talk = 0;
...
static int apds9190_do_calibration ( struct i2c_client *client, int *value )
{
    ...
    unsigned int new_ps_cross_talk = 0;
