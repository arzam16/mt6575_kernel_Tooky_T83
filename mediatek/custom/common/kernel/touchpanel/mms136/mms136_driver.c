...
static int tpd_keys_local[TPD_KEY_COUNT] = {KEY_MENU,KEY_HOME,KEY_BACK,KEY_SEARCH };
#endif
...
#if 0
static int mms_ts_fw_load(void *context)
{
...
//		tpd_fw_version = val[1];


		ret = tpd_i2c_read_bytes( i2c_client, 0x01, tp_config, 4);
...
int tpd_gettouchinfo(struct touch_info *cinfo) {
	...
	tpd_i2c_read_bytes(i2c_client,pkt_size,&pkt_size,1);
